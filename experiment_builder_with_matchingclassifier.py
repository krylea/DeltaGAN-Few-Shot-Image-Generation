...
class ExperimentBuilder(object):
    def __init__(self, args, data):
        ...
        if args.generation_layers == 6:
            ...
            # generator_layers = [128, 128, 256, 256]
        ...
        self.is_attention = tf.placeholder(tf.int32, name='is_attention')
        self.connection_layers = tf.placeholder(tf.int32, name='connection_layers')
        ...
        self.is_attention_value = args.is_attention
        self.connection_layers_value = args.connection_layers
        self.lr_g = args.lr_g
        self.lr_d = args.lr_d
        ...
        dagan = DAGAN(batch_size=self.batch_size, input_x_i=self.input_x_i, input_x_j=self.input_x_j,
                      ...
                      is_attention=self.is_attention_value, connection_layers=self.connection_layers_value)
        time_2 = time.time()
        print('time for defining graph:',time_2 - time_1)

        self.summary, self.losses, self.graph_ops = dagan.init_train(learning_rate_g=self.lr_g,
    ...
    def run_experiment(self):
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
            ...
            time_4 = time.time()
            sess.run(self.init)
            print('time for sess.run:', time.time() - time_4)
            ...
            log_name = "lrG{}_lrD{}_g{}_d{}_kl{}_cla{}_reconsZ{}_reconsB{}_matchingG{}_matchingD{}_sim{}_Net_batchsize{}_classencodedim{}_imgsize{}".format(
                self.lr_g, self.lr_d, self.loss_G, self.loss_D, self.loss_KL, self.loss_CLA, self.loss_FSL,
            ...
            if self.continue_from_epoch != -1:
                ...
                # for var in tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,scope='generator/unet'):
                # variables_to_restore.append(var)
                # for var in tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,scope='generator/generation'):
                # variables_to_restore.append(var)
            ...
            image_name = "lrG{}_lrD{}_g{}_d{}_kl{}_cla{}_reconsZ{}_ReconsFeature{}_StyleVerification{}_matchingD{}_ReconsImage{}_Net_batchsize{}_classencodedim{}_imgsize{}".format(
                self.lr_g, self.lr_d, self.loss_G, self.loss_D, self.loss_KL, self.loss_CLA, self.loss_FSL,
                self.loss_recons_B, self.loss_matching_G, self.loss_matching_D, self.loss_sim, self.batch_size,
                self.z_dim, self.image_width)

            if self.continue_from_epoch > 300:
                print('sampling')
                ...
                for e in range(start_from_epoch, self.total_epochs):
                            ...
                            for z1z2training in range(self.strategy):
                                ...
                                if z1z2training < -1:
                                    for n in range(self.gen_iter):
                                        ...
                                        print('time for constructing batch:', time.time() - before_classification)
                                        for i in range(1):
                                            ...
                                            _, g_train_loss_value, train_summaries = sess.run(
                                                ...
                                                feed_dict={
                                                    ...
                                                    self.is_attention: self.is_attention_value,
                                                    self.connection_layers: self.connection_layers_value})
                                ...
                                for j in range(1):
                                    ...
                                    if e % 10 == 0:
                                        _, _, _ = sample_generator(num_generations=self.num_generations,
                                                                   ...
                                                                   # input_global_x_j_selected = x_train_j_selected,
                                                                   # input_global_y_j_selected = y_global_train_j_selected,
                                                                   file_name="{}/test_z_variations_{}_{}_{}_reference.png".format(
                                                                       self.save_image_path,
                                                                       image_name,
                                                                       e, j),
                    ...
                    if e % 1 == 0 or e < 5:
                        train_save_path = self.train_saver.save(sess,
                                                                "{}/train_LOSS_Glr{}_Dlr{}_g{}_d{}_kl{}_cla{}_reconsZ{}_reconsB{}_matchingG{}_matchingD{}_sim{}_Net_batchsize{}_classencodedim{}_imgsize{}_epoch{}.ckpt".format(
                                                                    self.saved_models_filepath, self.lr_g, self.lr_d,
                                                                    self.loss_G, self.loss_D, self.loss_KL,
                    ...
                    if total_d_val_loss_mean < best_d_val_loss:
                        ...
                        val_save_path = self.train_saver.save(sess,
                                                              "{}/valid_LOSSg{}_d{}_kl{}_cla{}_reconsZ{}_ReconsFeature{}_StyleVerification{}_matchingD{}_ReconsImage{}_Net_batchsize{}_classencodedim{}_imgsize{}_epoch{}.ckpt".format(
                                                                  self.saved_models_filepath, self.loss_G, self.loss_D, self.loss_KL,
                                                                  self.loss_CLA, self.loss_FSL, self.loss_recons_B,
                                                                  self.loss_matching_G, self.loss_matching_D,
                                                                  self.loss_sim, self.batch_size, self.z_dim,
                                                                  self.image_width, e))
